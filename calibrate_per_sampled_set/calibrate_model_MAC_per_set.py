...
def run_efficacy_simulation_incidence(paramList, efficacy, doses, sub_iter):
    ...
    vacDoses = [doses,0]
    vacEfficacy_seasonal = efficacy
    vacEfficacy_universal = [0,0]
    ##set 1 for all hospitalization and death efficacy and scaling parameters. Calibrate these in step#2
    s = Simulation.run_Simulation(paramValues = {"vacEfficacy_universal":vacEfficacy_universal, "vacEfficacy_seasonal": vacEfficacy_seasonal, "betaList": paramList[0:3], "susceptibility_H1": paramList[3:7], "susceptibility_H3": paramList[7:11], "susceptibility_B": paramList[11:15],
...
def run_efficacy_simulation_burden(paramList, betalist, susc_list, efficacy, doses, sub_iter):
    ...
    s = Simulation.run_Simulation(paramValues = {"vacEfficacy_universal":vacEfficacy_universal, "vacEfficacy_seasonal": vacEfficacy_seasonal, "betaList": betalist, "susceptibility_H1": susc_list[0:4], "susceptibility_H3": susc_list[4:8], "susceptibility_B": susc_list[8:12], "vac_eff_hospitalization": paramList[0], "vac_eff_mortality": paramList[1], "prob_hosp_scaling": paramList[2], "prob_death_scaling": paramList[3]}, index = sub_iter, calibration = True)
    vaccineCoverage = s.compute_typical_vaccination(vacDoses)
    vacsUsedTypical, vacsUsedUniversal = s.simulateWithVaccine(vaccineCoverage, vacEfficacy_seasonal, vacEfficacy_universal, vacDoses)
    incidenceL, incidenceH, infections_H1, infections_H3, infections_B, perc_H1, perc_H3, perc_B,hospitalizationsL, hospitalizationsH, deathsL, deathsH = s.calibration_output()
...
def optimize(cons1, cons2, data_list_incidence,  sub_iter):
    ...
    return beta_H1, beta_H3, beta_B, H1_0, H1_5, H1_25, H1_65, H3_0, H3_5, H3_25, H3_65, B_0, B_5, B_25, B_65, eff_hosp, eff_death, hosp_scaling, death_scaling
...
def create_files():
	...
	myfile = open('DEBUG_results_calibrated_parameters.csv','wb')
...
if __name__ == "__main__":
	...
	df  = pd.read_csv("sampled_parameter_5000_set_with_data_20March2019.csv")
	year = df['year'].tolist()
	...
	for num in [4089]:
		...
		beta_H1, beta_H3, beta_B, H1_0, H1_5, H1_25, H1_65, H3_0, H3_5, H3_25, H3_65, B_0, B_5, B_25, B_65, eff_hosp, eff_death, hosp_scaling, death_scaling = optimize(cons1, cons2, data_list_incidence,  num)
