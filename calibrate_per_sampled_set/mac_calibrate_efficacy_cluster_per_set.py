...
def optimize(cons1, cons2, data_list,  sub_iter):
    ...
    for trial in xrange(1):
		...
		eff_opt_raw =  scipy.optimize.minimize(evaluateObjective_burden, BL0_burden, args=(beta_opt[0:3], beta_opt[3:], efficacy[sub_iter], total_doses[sub_iter], data_hospitalization[sub_iter], data_mortality[sub_iter], sub_iter), method='TNC', jac=None, bounds=bounds, constraints = {cons1, cons2}, tol=None, callback=None, options={'disp': None, 'maxls': 20, 'iprint': -1, 'gtol': 1e-05, 'eps': 1e-08, 'maxiter': 1500000, 'ftol': 2.220446049250313e-09, 'maxcor': 10, 'maxfun': 15000})
...
def create_files(num_files):
	...
	writer = {}
	header = ["iter", "incidence(millions)", "vac_eff_hospitalization", "vac_eff_mortality"]
...
##########################################################################
def start_pool(index, data_list, data_infections):
	
	eff_hosp, eff_death = optimize(cons1, cons2, data_list,  index)
	#	elem1 = [num, data_infections[num],eff_hosp, eff_death]
...
if __name__ == "__main__":
	...
	start_pool(index, data_list, data_infections)
