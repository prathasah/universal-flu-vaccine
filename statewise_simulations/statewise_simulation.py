...
import numpy
import sys
import statewise_vaccination_coverage
sys.path.insert(0, r'./Influenza/Parameters')
sys.path.insert(0, r'../Influenza/Parameters')
sys.path.insert(0, r'../../Influenza/Parameters')
import statewise_parameters as parameters
import matplotlib
matplotlib.use('Agg')

        
class run_Simulation:
    def __init__(self, options = None, tMin=0 , tMax = 365, paramValues = {}, state=None, index=None, calibration = False):
        self.tMax = tMax
	self.tMin = tMin
       ...
       # Parameters = __import__('Parameters', globals())
        ...
        self.parameters = parameters.Parameters(state,index, calibration, **paramValues)
    ...
    def compute_typical_vaccination(self, state, doses):
	    ...
	    vaccination_coverage_low_risk = statewise_vaccination_coverage.return_state_vax_coverage(state)
    ...
    def updateIC(self):
	    ...
	    # IUL_H1, IUL_H3, IUL_B
	    self.Y0[ 1: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 2: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 3: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    
	    # IUH_H1, IUH_H3, IUH_B
            self.Y0[ 8: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 9: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 10: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    
	    # ITL_H1, ITL_H3, ITL_B
	    self.Y0[ 15: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 16: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 17: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    
	    # ITH_H1, ITH_H3, ITH_B
	    self.Y0[ 22: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 23: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 24: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    
	    # INL_H1, INL_H3, INL_B
	    self.Y0[ 29: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 30: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 31: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    
	    # INH_H1, INH_H3, INH_B
	    self.Y0[ 36: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 37: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	    self.Y0[ 38: : 42] = numpy.full(self.parameters.ages.size, 0.001*self.parameters.population.sum())
	...
	self.totalCosts_H1 = self.parameters.state_expense_factor*(self._cost_overcounterMeds_H1 + self._cost_outpatient_H1 + self._cost_hospitalization_H1)
	...
	self.totalCosts_H3 = self.parameters.state_expense_factor*(self._cost_overcounterMeds_H3 + self._cost_outpatient_H3 + self._cost_hospitalization_H3)
	...
	self.totalCosts_B = self.parameters.state_expense_factor*(self._cost_overcounterMeds_B + self._cost_outpatient_B + self._cost_hospitalization_B)
	
	self.totalCosts = (self.totalCosts_H1 + self.totalCosts_H3 + self.totalCosts_B).sum()
	...
	relative_coverage_TL = [round(num/dosesVaccinatedTL[0],5) for num in dosesVaccinatedTL]
	relative_coverage_TH = [round(num/dosesVaccinatedTL[0],5) for num in dosesVaccinatedTH]
	relative_coverage_NL = [round(num/dosesVaccinatedNL[0],5) for num in dosesVaccinatedNL]
	relative_coverage_NH = [round(num/dosesVaccinatedNL[0],5) for num in dosesVaccinatedNH]
	
	#multiplication factor for final doses
	xFac_T = round(vacDoses[0]/(1.*(sum(relative_coverage_TL)+ sum(relative_coverage_TH))),5)
	xFac_N = round(vacDoses[1]/(1.*(sum(relative_coverage_NL)+ sum(relative_coverage_NH))),5)
	...
	self.doses_NH = [xFac_N * num for num in relative_coverage_NH]
        ...
        self.parameters.proportionVaccinatedTLPW.values = [round((a/(1.*b)),5) for a,b in zip(self.doses_TL,  self.parameters.population_lowrisk[1:])]
	self.parameters.proportionVaccinatedTHPW.values = [round((a/(1.*b)),5) for a,b in zip(self.doses_TH,  self.parameters.population_highrisk[1:])]
	self.parameters.proportionVaccinatedNLPW.values = [round((a/(1.*b)),5) for a,b in zip(self.doses_NL,  self.parameters.population_lowrisk[1:])]
	self.parameters.proportionVaccinatedNHPW.values = [round((a/(1.*b)),5) for a,b in zip(self.doses_NH,  self.parameters.population_highrisk[1:])]
	...
	#plt.plot(times, (self.INL_H1).sum(axis=1)/self.parameters.population.sum(), color = "red", linestyle= "-")
	#plt.plot(times, (self.INL_H3).sum(axis=1)/self.parameters.population.sum(), color = "blue", linestyle= "-")
	#plt.plot(times, (self.INL_B).sum(axis=1)/self.parameters.population.sum(), color = "green", linestyle = "-")	 
	#plt.savefig(state+"_results.png")
	#plt.clf()
    ...
    def basic_output_per_capita(self):
	return self.totalInfections/self.parameters.population.sum(), self.totalHospitalizations/self.parameters.population.sum(), self.totalDeaths/self.parameters.population.sum(), self.totalCosts/self.parameters.population.sum()
