import statewise_demography
import statewise_expense_factor
import sys
...
##################################################################
    ...
    def __init__(self, state, index, calibration, **paramValues):
	...
	self.setPWAttr("populationPW", statewise_demography.return_state_demography(state))
	setattr(self,"state_expense_factor", statewise_expense_factor.return_state_expense(state))
	...
	self.proportionVaccinatedHLength = len(vaccinationAgesTypical)
        ...
        if 'contactMatrix' in paramValues:
            ...
            import os.path
            import cPickle
            modulePath = os.path.abspath(os.path.join(os.path.dirname(modules[self.__module__].__file__),".."))
            contactMatrixFile =os.path.join(modulePath, 'Influenza/Parameters/contactMatrix.p')
