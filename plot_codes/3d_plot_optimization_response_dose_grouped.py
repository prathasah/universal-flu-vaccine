...
def plot_3d_figure(df, ax1, title):
	...
	for num2 in xrange(x.shape[1]):
	    xnum = x[num1][num2]
	    ynum = y[num1][num2]
	    dz1[num1][num2] = df[(df['age_group'] == xnum) & (df['scenario'] == ynum) & (df["type"] == "low_efficacy")]["doses"]
	    dz2[num1][num2] = df[(df['age_group'] == xnum) & (df['scenario'] == ynum) & (df["type"] == "high_efficacy")]["doses"]
    ...
    ax1.set_zlabel('Doses (millions)', fontsize=18)
...
dt1 = pd.read_csv("../data/optimization_results/cleaned_optimization_infection_response_doses_100iter.csv")
s1 = fig.add_subplot(221, projection='3d')
title = "Minimizing incidence"
plot_3d_figure(dt1, s1, title)

dt2 = pd.read_csv("../data/optimization_results/cleaned_optimization_hospitalization_response_doses_100iter.csv")
print ("hospitalization")
s2 = fig.add_subplot(222, projection='3d')
title = "Minimizing hospitalization"
plot_3d_figure(dt2, s2, title)

dt3 = pd.read_csv("../data/optimization_results/cleaned_optimization_death_response_doses_100iter.csv")
print ("death")
s3 = fig.add_subplot(223, projection='3d')
title = "Minimizing mortality"
plot_3d_figure(dt3, s3, title)

dt4 = pd.read_csv("../data/optimization_results/cleaned_optimization_DALY_response_doses_100iter.csv")
...
plt.savefig("../plots/minimizing_burden_response_dose.png",  pad_inches=0)
#plt.show()
